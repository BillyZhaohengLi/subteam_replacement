...
def teamrep_fast_exact_edge_labeled(A, L, W, c, current_team, remaining_team):
    l = W.shape[0]
    t1 = len(current_team)
    ...
    for m in range(l):
        for n in range(l):
            E1 = W[m][n] * np.matmul(L1[:,m,None], np.transpose(L1[:,n,None]))
    ...
    for m in range(l):
        for n in range(l):
            Ec = np.matmul(Lc[:,m,None], np.transpose(Lc[:,n,None]))
            Y = np.multiply(Ec, Ac)
...
## current_team: original team T
## remaining_team: remaining team T - p
def teamrep_fast_exact_edge_labeled_undirected(A, L, W, c, current_team, remaining_team):
    l = W.shape[0]
    t1 = len(current_team)
    t2 = len(remaining_team)
    ...
    for m in range(l):
        for n in range(m + 1):
            E1 = W[m][n] * (np.matmul(L1[:,m,None], np.transpose(L1[:,n,None])) + np.matmul(L1[:,n,None], np.transpose(L1[:,m,None])))
            Z[m][n] = np.multiply(E1, A1)
    ...
    for m in range(l):
        for n in range(m + 1):
            Ec = np.matmul(Lc[:,m,None], np.transpose(Lc[:,n,None])) + np.matmul(Lc[:,n,None], np.transpose(Lc[:,m,None]))
            Y = np.multiply(Ec, Ac)
            Ablock -= c * np.kron(Y, Z[m][n])
    ...
    for candidate in candidates:
        ...
        for m in range(l):
            for n in range(m + 1):
                Ed1 = Lc[:,m,None] * Ld1[:,n] + Lc[:,n,None] * Ld1[:,m]
                X = np.multiply(Ed1, Ad2)
                Bblock -= c * np.kron(X, Z[m][n])
...
def teamrep_fast_exact_edge_labeled_argmax(A, L, W, c, current_team, remaining_team):
    l = W.shape[0]
    t1 = len(current_team)
    t2 = len(remaining_team)
